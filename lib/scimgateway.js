...
let ScimGateway = function () {
  ...
  // Initial connection, step #1: GET /ServiceProviderConfig
  // If not included => Provisioning will always use GET /Users without any paramenters
  app.get('(|/:baseEntity)(|/scim)/ServiceProviderConfig', function (req, res) {
  ...
  app.get('(|/:baseEntity)(|/scim)/ResourceType', function (req, res) {
  ...
  app.get(`(|/:baseEntity)(|/scim)/(!${undefined}|Users|Groups|servicePlans)`, (req, res) => {
    ...
    // should serve /Uses /Groups ...
    // so only check if there is a filter
    if (!req.query.filter) {
      // ==========================================
      //             EXPLORE
      // ==========================================
      ...
      if (obj === null) {
        ...
        this.emit(handle.createEmitter, req.params.baseEntity, obj, function (err) {
          if (err) {
            ...
            // should delete password from response
  ...
  // Note, using id (not groupname). Explore should therefore set id = groupname (groupID)
  // We then have: DELETE /Groups/bjensen
  //
  ...
  app.patch(`(|/:baseEntity)(|/scim)/(!${undefined}|Users|Groups|servicePlans)/:id`, (req, res) => {
    ...
    req.on('end', () => {
      ...
      const self = this;
      self.emit(handle.modifyEmitter, req.params.baseEntity, id, scimdata.members ? scimdata.members : scimdata, function (err) {
        if (err) {
          err = jsonErr(config.scimversion, pluginName, '500', err)
          res.status(500).send(err)
          ...
          self.emit(handle.getEmitter, req.params.baseEntity, id, req.query.attributes ? req.query.attributes : '', function (err, data) {
            let user = Object.assign({ id }, data, scimdata);
            res.setHeader('Location', `${location}`)
